use client_command::client_command::{countDownTime, Room};
mod client_command;
use std::{
    collections::HashMap,
    env,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    room_map.lock().unwrap().insert(
        "QuickPlay".to_string(),
        Room::new("QuickPlay".to_string(), Vec::new()),
    );
...
async fn process_connection(
    peer_map: PeerMap,
    RoomMap: Arc<Mutex<HashMap<String, Room>>>,
    ...
    let broadcast_incoming = incoming.try_for_each(|msg| {
        ...
        let command = serde_json::from_str::<client_command::client_command::ClientCommand>(
            msg.to_text().unwrap_or("Error"),
        )
        .unwrap_or(client_command::client_command::ClientCommand::Error);
        //println!("{:?}", command);
        ...
        match command {
            ClientCommand::RegPlayer(string) if !registered => {
                ...
                if count == 0 && string.len() > 1 {
                    ...
                    let message = Message::Text("RegPlayer".to_string().into());
                    peer_map
                        .lock()
                        .expect("Peer map lock error 112")
                        .get(&addr)
                        .expect("Get error 112")
                        .unbounded_send(message)
                        .expect("Send error 112");
                    name = string;
                    registered = true;
                } else {
                    let message = Message::Text("NameTaken".to_string().into());
                    peer_map
                        .lock()
                        .expect("Peer map lock error 117")
                        .get(&addr)
                        .expect("Get error 117")
                        .unbounded_send(message)
                        .expect("Send error 117");
                }
            }
            ClientCommand::JoinRoom(string) if registered && current_room == *"" => {
                ...
                match room_map {
                    Some(x) => {
                        if x.isStarted {
                            let message = Message::Text("RoomStarted".to_owned().into());
                            peer_map
                                .lock()
                                .expect("Peer lock error 129")
                                .get(&addr)
                                .expect("Peer lock error 129")
                                .unbounded_send(message)
                                .expect("Send error 129");
                        } else {
                            ...
                            if let Some(thing) = pos {
                                ...
                                x.players[socket_index] =
                                    client_command::client_command::Player::new(name.clone(), addr);
                            } else if current_room != string {
                                socket_index = x.players.len();

                                x.players.push(client_command::client_command::Player::new(
                                    name.clone(),
                                    addr,
                                ));
                            }
                            ...
                            if !x.isReady {
                                ...
                                println!("Dropping x");
                                drop(x);
                                tokio::spawn(gameProccessThread(
                                    current_room.clone(),
                                    peer_map.clone(),
                                    RoomMap.clone(),
                                ));
                            }
                            let message = Message::Text("RoomJoined".to_owned().into());
                            peer_map
                                .lock()
                                .expect("Peer lock error 160")
                                .get(&addr)
                                .expect("Get error 160")
                                .unbounded_send(message)
                                .expect("Send error 160");
                        }
                    }
                    _ => {
                        let message = Message::Text("RoomDoesNotExist".to_owned().into());
                        peer_map
                            .lock()
                            .expect("Peer lock error 164")
                            .get(&addr)
                            .expect("Get error 164")
                            .unbounded_send(message)
                            .expect("Get error 164");
                    }
                }
                ...
                match room_map {
                    Some(x) => {
                        if x.isStarted {
                            ...
                            peer_map
                                .lock()
                                .expect("Lock error 200")
                                .get(&addr)
                                .expect("Get error 200")
                                .unbounded_send(message)
                                .expect("Send error 200");
                        } else {
                            let pos = x.players.iter().position(|p| p.name.len() <= 1);
                            if let Some(thing) = pos {
                                if current_room != string {
                            ...
                            peer_map
                                .lock()
                                .expect("Peer Lock error 229")
                                .get(&addr)
                                .expect("Get error 229")
                                .unbounded_send(message)
                                .expect("Send error 229");
                        }
                    }
                    _ => {
                        ...
                        if let Some(x) = guard.get_mut("QuickPlay") {
                            ...
                            peer_map
                                .lock()
                                .expect("Peer map lock 254")
                                .get(&addr)
                                .expect("Get error 254")
                                .unbounded_send(message)
                                .expect("Send Error 254");
                        } else {
                            println!("QuickPlay not found 256");
                        }
                    }
                }
            }
                ...
                match room {
                    Some(x) => {
                        ...
                        let players: Vec<&Player> =
                            x.players.iter().filter(|x| x.name.len() > 1).collect();
                        let serde = serde_json::to_string(&players).expect("Parse error 269");
                        let message = Message::Text(("Data!".to_owned() + &serde).into());
                        //println!("{:?}", message);
                        peer_map
                            .lock()
                            .expect("Peer lock 272")
                            .get(&addr)
                            .expect("Get error 272")
                            .unbounded_send(message)
                            .expect("Send error 272");
                    }
                    _ => {
                        println!("Room not found 275");
                    }
                }
            }
            ClientCommand::GetObstacles if registered && current_room != *"" => {
                ...
                match room {
                    Some(x) => {
                        ...
                        let message = Message::Text(("Obstacles!".to_owned() + &serde).into());
                        //println!("{:?}", message);
                        peer_map
                            .lock()
                            .expect("Peer lock error 292")
                            .get(&addr)
                            .expect("Get error 292")
                            .unbounded_send(message)
                            .expect("Send error 292");
                    }
                    _ => {
                        println!("Room not found 295");
                    }
                }
            }
                ...
                match room {
                    Some(x) => {
                        ...
                        match player {
                            ...
                            _ => {
                                println!("Player not found 306");
                            }
                        }
                    }
                    _ => {
                        println!("Room not found 309");
                    }
                }
            }
            ...
            ClientCommand::GetRoomList => {
                ...
                let serde = serde_json::to_string(&guard.keys().collect::<Vec<&String>>())
                    .expect("Parse Error 335");
                let message = Message::Text(("Rooms!".to_owned() + &serde).into());
                peer_map
                    .lock()
                    .expect("Peer lock error 337")
                    .get(&addr)
                    .expect("Get error 337")
                    .unbounded_send(message)
                    .expect("Send error 337");
            }
            ClientCommand::CreateRoom(x) if registered && current_room == *"" => {
                let mut guard = RoomMap.lock().expect("Room map Lock error 341");
                if x.len() < 1 {
                    //room name is less than one -- invalid
                    let message = Message::Text("Invalid Room Name".to_string().into());
                    peer_map
                        .lock()
                        .expect("Peer Map lock error 460")
                        .get(&addr)
                        .expect("Can't find addr 460")
                        .unbounded_send(message)
                        .expect("Send error 460");
                } else {
                    let room = guard.get_mut(&x);
                    match room {
                        Some(y) => {
                            //TODO: Create ROOM
                            let message = Message::Text("RoomAlreadyExists".to_string().into());
                            peer_map
                                .lock()
                                .expect("Peer Map lock error 347")
                                .get(&addr)
                                .expect("Can't find addr 347")
                                .unbounded_send(message)
                                .expect("Send error 347");
                        }
                        _ => {
                            let y = Room::new(x.clone(), Vec::new());
                            guard.insert(x.clone(), y);
                            let message = Message::Text("RoomCreated".to_string().into());
                            peer_map
                                .lock()
                                .expect("Peer Map error 353")
                                .get(&addr)
                                .expect("Can't find addr 353")
                                .unbounded_send(message)
                                .expect("Send error 353");
                        }
                    }
                }
            }
    ...
    if let Some(room) = RoomMap
        .lock()
        .expect("Room Lock error")
        .get_mut(&current_room)
    {
        ...
        if room.players.len() - 1 == socket_index {
            ...
            let mut popdex = 0;
            room.players.iter().rev().for_each(|x| {
                if working && x.name.len() <= 1 {
                    popdex += 1;
                } else {
                    working = false;
                }
        ...
        println!(
            "{}, {}",
            room.players.len(),
            peer_map.lock().expect("Peer map error 392").len()
        );
        let count = room.players.iter().position(|x| x.name != "".to_string());
        match count {
            Some(_) => {
                println!("Room empty 395");
            }
            None => room.players.retain(|_| false),
        }
    } else {
        println!("Room not found 399");
    }
    println!("Names removing 400");
    names
        .lock()
        .expect("Can't lock names 401")
        .retain(|x| x != &name);
}

async fn gameProccessThread(
    cur_room: String,
    peer_map: PeerMap,
    RoomMap: Arc<Mutex<HashMap<String, Room>>>,
) {
        ...
        if true {
            ...
            if let Ok(mut x) = rooms {
                if let Some(y) = x.get(&cur_room) {
                    ...
                    println!("Room not found 442");
                }
            } else {
                println!("Map not found 443");
            }
        }
        if waiting {
            println!("WAITING, 349");
            ...
            totalTime -= 1;
            let message = Message::Text(("Countdown!".to_owned() + &serde).into());
    ...
    loop {
        let mut message = Message::Text("Data! error".to_owned().into());
        let mut len = -1;
...

        {
            ...
            match room {
                Some(x) => {
                    ...
                    message = Message::Text(("Data!".to_owned() + &serde).into());
                    //println!("{:?}", message);
        ...
        if len == 0 {
            ...
            pres_broadcast(
                peer_map.clone(),
                Message::Text(("GameOver!".to_owned() + &winner).into()),
                &addrVec,
            )
            .await;
            break;
...
async fn broadcast(peer_map: PeerMap, msg: Message) {
    ...
    for recp in broadcast_recipients {
        recp.unbounded_send(msg.clone())
            .expect("Recp Broadcast Error 596");
    }
}
...
async fn pres_broadcast(peer_map: PeerMap, msg: Message, addrVec: &Vec<SocketAddr>) {
    ...
    let broadcast_recipients = peers
        .iter()
        .filter(|x| addrVec.contains(x.0))
        .map(|(_, ws_sink)| ws_sink);

    for recp in broadcast_recipients {
        recp.unbounded_send(msg.clone())
            .expect("Recp Broadcast Error 612");
    }
}
